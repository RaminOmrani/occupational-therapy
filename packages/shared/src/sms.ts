...
export const SMS_TEMPLATE_DEFAULTS: SmsTemplateDef[] = [
  ...
  },
  {
    ...
    variables: ["name", "date", "list", "clinic"],
    patternArgs: ["name", "date", "list"],
    description: "وقتی مراجع در یک روز چند نوبت دارد (مثلاً با دو درمانگر). اگر کد الگو نداشته باشد، از الگوی «قطعی‌شدن نوبت» با ساعت‌ها و درمانگران ترکیبی استفاده می‌شود.",
  },
  {
    key: "no_show",
    name: "پیگیری غیبت",
    body: "{{name}} عزیز، در جلسه {{date}} حضور نداشتید. برای تعیین نوبت جدید با {{phone}} تماس بگیرید.\n{{clinic}}",
    variables: ["name", "date", "phone", "clinic"],
    patternArgs: ["name", "date", "phone"],
    description: "وقتی نوبتی «غیبت» ثبت شود (در صورت فعال‌بودن در تنظیمات).",
  },
  {
    key: "booking_received",
    name: "دریافت درخواست نوبت آنلاین",
    body: "{{name}} عزیز، درخواست نوبت شما برای {{date}} ساعت {{time}} ثبت شد. پس از تأیید، پیامک نهایی ارسال می‌شود.\n{{clinic}}",
    variables: ["name", "date", "time", "clinic"],
    patternArgs: ["name", "date", "time"],
    description: "بلافاصله پس از ثبت رزرو آنلاین در سایت.",
  },
  {
    key: "appointment_rescheduled",
    name: "جابه‌جایی نوبت",
    body: "{{name}} عزیز، نوبت شما به {{date}} ساعت {{time}} با {{therapist}} تغییر کرد.\n{{clinic}}",
    variables: ["name", "date", "time", "therapist", "clinic"],
    patternArgs: ["name", "date", "time", "therapist"],
    description: "وقتی زمان یا درمانگرِ نوبت قطعی‌شده تغییر کند (در صورت فعال‌بودن در تنظیمات).",
  },
  {
    key: "invoice_issued",
    name: "صدور صورت‌حساب",
    body: "{{name}} عزیز، صورت‌حساب {{number}} به مبلغ {{amount}} {{currency}} صادر شد. مهلت پرداخت: {{due}}.\n{{clinic}}",
    variables: ["name", "number", "amount", "currency", "due", "clinic"],
    patternArgs: ["name", "number", "amount", "due"],
    description: "هنگام صدور صورت‌حساب (در صورت فعال‌بودن در تنظیمات).",
  },
  {
    key: "waitlist_slot",
    name: "خالی‌شدن نوبت برای لیست انتظار",
    body: "{{name}} عزیز، نوبت {{date}} ساعت {{time}} با {{therapist}} خالی شد. برای رزرو با {{phone}} تماس بگیرید.\n{{clinic}}",
    variables: ["name", "date", "time", "therapist", "phone", "clinic"],
    patternArgs: ["name", "date", "time", "therapist", "phone"],
    description: "وقتی نوبتی لغو می‌شود، به نفر اول لیست انتظار همان درمانگر ارسال می‌شود.",
  },
  {
