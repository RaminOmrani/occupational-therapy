...
export const DEFAULT_SETTINGS: Record<string, { value: string; group: string; label: string; description?: string; type: "text" | "number" | "boolean" | "json" | "password" | "textarea" | "select" | "image"; options?: string[]; secret?: boolean }> = {
  ...
  "clinic.about": { value: "کلینیک توان‌بخشی ذهن سبز 💚 در مشهد با تیمی از درمانگران مجرب، خدمات توان‌بخشی جسمی، ذهنی و شناختی را برای کودکان و بزرگسالان ارائه می‌دهد.", group: "clinic", label: "درباره ما", type: "textarea" },
  "clinic.mapUrl": { value: "", group: "clinic", label: "لینک نقشه", type: "text" },
  "clinic.services": {
    value: JSON.stringify([
      { title: "توان‌بخشی جسمی", description: "بهبود دامنه حرکتی، قدرت عضلانی و مهارت‌های حرکتی ظریف و درشت", icon: "activity" },
      { title: "توان‌بخشی ذهنی و شناختی", description: "تقویت توجه، حافظه، حل مسئله و کارکردهای اجرایی", icon: "brain" },
  ...
  "schedule.endHour": { value: "22", group: "schedule", label: "ساعت پایان کار", type: "number" },
  ...
  "finance.cardNumber": { value: "", group: "finance", label: "شماره کارت کلینیک (برای کارت‌به‌کارت)", type: "text", description: "به مراجعین در بخش مالی نمایش داده می‌شود" },
  "finance.cardOwner": { value: "", group: "finance", label: "نام صاحب کارت", type: "text" },
  "finance.paymentNote": { value: "پس از واریز، از بخش «اعلام پرداخت» مبلغ و شماره پیگیری را ثبت کنید تا پس از تأیید پذیرش در حساب شما منظور شود.", group: "finance", label: "راهنمای پرداخت برای مراجع", type: "textarea" },
  ...
  "sms.autoReschedule": { value: "true", group: "sms", label: "پیامک «نوبت شما جابه‌جا شد» وقتی زمان نوبت قطعی تغییر کند", type: "boolean" },
  ...
  "sms.sharedLine": { value: "true", group: "sms", label: "خط خدماتی اشتراکی (فقط الگو)", type: "boolean", description: "اگر فعال باشد، ارسال متن آزاد انجام نمی‌شود و هر الگو باید «کد الگو» داشته باشد. برای خط اختصاصی غیرفعال کنید." },
  "sms.signature": { value: "کلینیک توان‌بخشی ذهن سبز 💚", group: "sms", label: "امضای انتهای پیامک‌ها", type: "text" },
  ...
  "sms.autoWelcome": { value: "true", group: "sms", label: "ارسال پیامک خوش‌آمد به مراجع جدید", type: "boolean" },
  ...
  "booking.autoSms": { value: "true", group: "booking", label: "پیامک به مراجع پس از تأیید درخواست", type: "boolean" },
  ...
  "consent.required": { value: "true", group: "consent", label: "دریافت رضایت‌نامه از مراجعین جدید الزامی باشد", type: "boolean" },
  "consent.title": { value: "رضایت‌نامه آگاهانه درمان", group: "consent", label: "عنوان رضایت‌نامه", type: "text" },
  "consent.text": { value: "اینجانب با آگاهی کامل از روند درمان توان‌بخشی، اهداف، مدت و هزینه‌های آن، رضایت خود را برای شروع و ادامه درمان در کلینیک اعلام می‌دارم.\nمتعهد می‌شوم در جلسات به‌موقع حاضر شوم و در صورت لغو، حداقل ۲۴ ساعت قبل اطلاع دهم.\nاطلاعات پرونده محرمانه است و فقط در اختیار تیم درمان قرار می‌گیرد.", group: "consent", label: "متن رضایت‌نامه", type: "textarea", description: "با تغییر متن، مراجعین باید نسخه جدید را دوباره امضا کنند." },
  ...
  "public.mediaTitle": { value: "رسانه ذهن سبز", group: "public", label: "نام بخش محتوا (مقاله، ویدیو، پادکست، کتاب)", type: "text", description: "در منوی سایت و صفحه محتوا نمایش داده می‌شود" },
  "public.mediaSubtitle": { value: "مقاله، ویدیو، پادکست و کتاب؛ از تیم درمانگران و منابع معتبر", group: "public", label: "زیرعنوان بخش محتوا", type: "text" },
  "public.stats.enabled": { value: "true", group: "public", label: "نمایش نوار آمار و افتخارات در سایت", type: "boolean" },
  "public.stats.years": { value: "", group: "public", label: "سال‌های تجربه", type: "number", description: "خالی = نمایش داده نمی‌شود" },
  "public.stats.patients": { value: "", group: "public", label: "تعداد مراجعین", type: "number", description: "خالی = خودکار از تعداد پرونده‌ها" },
  "public.stats.sessions": { value: "", group: "public", label: "تعداد جلسات درمانی", type: "number", description: "خالی = خودکار از جلسات انجام‌شده" },
  "public.stats.satisfaction": { value: "", group: "public", label: "درصد رضایت مراجعین", type: "number", description: "خالی = خودکار از امتیاز نظرسنجی‌ها" },
  "public.stats.therapists": { value: "", group: "public", label: "تعداد درمانگران", type: "number", description: "خالی = خودکار از درمانگران فعال" },
  "public.stats.extraLabel": { value: "", group: "public", label: "آمار دلخواه: عنوان", type: "text", description: "مثلاً «کارگاه آموزشی برگزارشده»" },
  "public.stats.extraValue": { value: "", group: "public", label: "آمار دلخواه: مقدار", type: "text" },
  "public.heroSubtitle": { value: "کلینیک تخصصی توان‌بخشی کودکان و بزرگسالان؛ ارزیابی دقیق، برنامه درمانی شخصی و پیگیری مستمر پیشرفت", group: "public", label: "زیرعنوان صفحه نخست", type: "textarea" },
};
