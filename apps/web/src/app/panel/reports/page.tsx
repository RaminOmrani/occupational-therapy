...
export default function ReportsPage() {
  ...
  const smsAtRisk = async () => {
    ...
    try { await api.post("/sms/campaigns", { name: `پیگیری مراجعین غایب (${toPersianDigits(riskDays)} روز)`, message: "{{name}} عزیز، مدتی است در جلسات توان‌بخشی حضور نداشته‌اید. برای ادامه روند درمان و تعیین نوبت با ما تماس بگیرید.", filters: { audience: "patients", patientIds: data.atRisk.map((p: any) => p.id) } }); toast.success("پیامک پیگیری برای مراجعین در خطر ریزش ارسال شد"); } catch (e: any) { toast.error(e.message); }
  };
  ...
  return (
    <>
      <PageHeader title="گزارش‌های مدیریتی" subtitle="نرخ تبدیل، غیبت، عملکرد درمانگران و مراجعین در خطر ریزش" icon={<BarChart3 className="h-5 w-5" />} actions={<><JalaliDatePicker value={from} onChange={setFrom} className="w-36" /><JalaliDatePicker value={to} onChange={setTo} className="w-36" /></>} />
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat label="نرخ تبدیل لید به مراجع" value={`${toPersianDigits(data.leads.rate)}٪`} icon={<UserPlus className="h-6 w-6" />} hint={`${toPersianDigits(data.leads.converted)} از ${toPersianDigits(data.leads.total)} لید`} />
        ...
        <Stat label="مراجعین جدید" value={toPersianDigits(data.newPatients)} icon={<Users className="h-6 w-6" />} tone="sage" hint={`${toPersianDigits(data.appointments.webBookings)} نوبت از سایت`} />
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <Card title="عملکرد درمانگران" subtitle="در بازه انتخابی" padded={false} className="overflow-x-auto">
          <table className="table"><thead><tr><th>درمانگر</th><th>جلسات</th><th>مراجعین</th><th>غیبت</th><th>لغو</th><th>درآمد جلسات</th></tr></thead>
      ...
      <Card className="mt-5" title="مراجعین در خطر ریزش" subtitle="مراجعین فعال بدون نوبت آینده که مدتی مراجعه نکرده‌اند" actions={<><Select value={riskDays} onChange={(e) => setRiskDays(Number(e.target.value))} className="w-36">{[14, 30, 45, 60, 90].map((d) => <option key={d} value={d}>{toPersianDigits(d)} روز</option>)}</Select><Button size="sm" variant="secondary" onClick={smsAtRisk} disabled={!data.atRisk.length} icon={<MessageSquareText className="h-4 w-4" />}>پیامک پیگیری به همه</Button></>} padded={false}>
        ...
        ) : <EmptyState title="مراجعی در خطر ریزش نیست 🎉" />}
      </Card>
    </>
  );
}
