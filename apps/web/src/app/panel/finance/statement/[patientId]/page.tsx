  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="card p-8 print:border-0 print:shadow-none">
        ...
        <div className="mt-6 flex justify-end"><p className="text-base">مانده نهایی: <b className={`num ${data.balance > 0 ? "text-coral-600" : "text-sage-700"}`}>{data.balance === 0 ? "تسویه‌شده" : `${formatMoney(Math.abs(data.balance))} ${data.balance > 0 ? "بدهکار" : "بستانکار"}`}</b></p></div>
      </div>
    </div>
  );
}
