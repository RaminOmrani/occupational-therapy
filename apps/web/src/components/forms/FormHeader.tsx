...
/** بخش مشترک بالای فرم‌های بالینی: مراجع، درمانگر، تاریخ، نمایش به مراجع */
export function FormHeaderFields({ value, onChange, locked }: { value: FormHead; onChange: (v: FormHead) => void; locked?: boolean }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <Field label="مراجع" required className="lg:col-span-2"><PatientPicker value={value.patientId} initialLabel={value.patientLabel} disabled={locked} onChange={(id, p) => onChange({ ...value, patientId: id ?? "", patientLabel: p ? `${p.fullName} (${p.fileNumber})` : "", therapistId: value.therapistId || p?.primaryTherapistId || "" })} /></Field>
      {user?.role !== "THERAPIST" && (
      ...
      <div className="flex items-end pb-2 sm:col-span-2 lg:col-span-4"><Toggle checked={value.visibleToPatient} onChange={(c) => onChange({ ...value, visibleToPatient: c })} label="نمایش این فرم در پنل مراجع" /></div>
    </div>
  );
}
