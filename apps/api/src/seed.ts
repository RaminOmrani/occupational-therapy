 ...
 *   مراجع:    09120000010 / OT-00001 (یا ورود با کد پیامکی)
 */
...
async function main() {
  ...
  const t1 = await prisma.therapist.upsert({
    ...
    create: { userId: t1User.id, specialty: "توان‌بخشی جسمی و ذهنی", bio: "کاردرمانگر با تجربه در توانبخشی کودکان و بزرگسالان", licenseNo: "ک.د ۲۴۳۰", color: "#178a6e", sessionPrice: 550000, sortOrder: 1 },
    update: {},
  });
  const t2 = await prisma.therapist.upsert({
    where: { userId: t2User.id },
    create: { userId: t2User.id, specialty: "توان‌بخشی ذهنی و یکپارچگی حسی", bio: "کاردرمانگر با تمرکز بر مهارت‌های شناختی و ادراکی-حرکتی کودکان", licenseNo: "", color: "#E76F51", sessionPrice: 600000, sortOrder: 2 },
    update: {},
  });
  ...
  if (existing === 0) {
    ...
    for (const s of samples) {
      ...
      const items = sessions.map(() => ({ title: "جلسه توان‌بخشی", qty: 1, unitPrice: s.therapist.sessionPrice ?? 500000 }));
    ...
    for (const l of [
      { firstName: "حسین", lastName: "پاکدل", phone: "09120000020", source: "WEBSITE", status: "NEW", interest: "توان‌بخشی کودک ۴ ساله با تأخیر گفتاری" },
  ...
  await setSetting("clinic.name", "کلینیک توان‌بخشی ذهن سبز 💚");
  ...
  console.log("   مراجع:   09120000010 / OT-00001 (یا کد پیامکی)");
}
